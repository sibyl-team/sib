...
struct Message : public std::vector<T>
{
	...
	void clear()
	{
		for (int i = 0; i < int(std::vector<T>::size()); ++i)
			std::vector<T>::operator[](i) *= 0.0;
	}
	void init_rand()
	{
		for (int i = 0; i < int(std::vector<T>::size()); ++i)
		{
			std::vector<T>::operator[](i) = (1. * rand()) / RAND_MAX;
		}
	}
	void init_uniform()
	{
		for (int i = 0; i < int(std::vector<T>::size()); ++i)
			std::vector<T>::operator[](i) = 1.;
	}
	...
	inline T &val(int sji, int sij) { return this->operator()(sji, sij); }
	size_t qj;
};
...
struct Node
{
	Node(std::shared_ptr<Proba> prob_i, std::shared_ptr<Proba> prob_r, int index) : prob_i(prob_i),
																					prob_r(prob_r),
																					prob_i0(prob_i),
																					prob_r0(prob_r),
																					f_(0),
