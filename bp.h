...
public:
	...
	FactorGraph(Params const & params,
		...
		std::vector<std::tuple<int, times_t, std::shared_ptr<Test>>> const & obs,
	...
	void append_observation(int i, times_t t, std::shared_ptr<Test> const & o);
	...
	void reset_observations(std::vector<std::tuple<int, times_t, std::shared_ptr<Test>>> const & obs);
